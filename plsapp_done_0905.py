...
class PLSApp:
    ...
    def plot_train(self):
        ...
        selected_columns = list(range(1, 30))
        excluded_columns = list(range(0, 0))
        X_selected = self.data.iloc[:, selected_columns]
        X_selected = X_selected.drop(X_selected.columns[excluded_columns], axis=1)
        feature_names = X_selected.columns

        plt.figure(figsize=(8, 4.5))
        with plt.style.context(('ggplot')):
            plt.plot(feature_names, X_selected.T)
